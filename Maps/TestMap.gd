...
func _on_player_list_changed():
	...
	for child in $HUD/PlayerListPanel/RemotePlayerVBox.get_children():
		child.queue_free()
	
	# Now iterate through the player list creating a new entry into the boxList
	for parent in Network.players:
		if (parent != GameState.player_info.net_id):
			var new_label = Label.new()
			new_label.text = Network.players[parent].name
			$HUD/PlayerListPanel/RemotePlayerVBox.add_child(new_label)
...
func _ready():
	...
	$HUD/PlayerListPanel/LocalPlayerLabel.text = GameState.player_info.name
